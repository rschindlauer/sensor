...
bool RFM69Manager::initialize(uint8_t frequency, uint8_t nodeID, uint8_t networkID, const char* key, uint8_t gatewayID, int16_t targetRSSI) {
    ...
    RFM69_ATC::initialize(frequency, nodeID, networkID);
    encrypt(key);
...
bool RFM69Manager::loop() {
    ...
    if (receiveDone()) {
        ...
        uint8_t packetID = 0;
        char * payload = strtok(buffer, sep);
        if (parts > 1) {
...
bool RFM69Manager::send(uint8_t destinationID, char * payload, uint8_t retries, bool requestACK) {

    char message[RF69_MAX_DATA_LEN];

    #if SEND_PACKET_ID
        if (++_sendCount == 0) _sendCount = 1;
        snprintf(message, RF69_MAX_DATA_LEN-1, "%s%c%d", payload, _separator, _sendCount);
    #else
        snprintf(message, RF69_MAX_DATA_LEN-1, "%s", payload);
    #endif
