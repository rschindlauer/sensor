...
class RFM69Manager: public RFM69_ATC {
    ...
    public:
        // These defaults are defined in RFM69.h
        ...
        bool send(uint8_t destinationID, char * payload, uint8_t retries = RETRIES, bool requestACK = REQUESTACK);
        bool send(char * payload, uint8_t retries = RETRIES) {
            return send(_gatewayID, payload, retries, false);
        }
        bool send(char * payload, bool requestACK = REQUESTACK) {
            return send(_gatewayID, payload, 0, requestACK);
        }
        bool loop();
